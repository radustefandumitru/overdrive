...
| Liquid Glass Web | Progressive web glass UI: universal frosted glass, Chromium SVG displacement, and optional license-checked WebGL refraction | Yes; local original skill with cross-browser fallback guidance | Taste, Emil, `fluid-animations`, `modern-web-guidance`, `playwright-cli` | Do not vendor liquid-dom, kube.io snippets, or WebGL libraries without license review; verify target browser support |
...
| Layers of Product Design | Product-design reasoning across observed behaviour, domain, user needs, strategy, conceptual model, interaction flow, and surface decisions | Yes; MIT `SKILL.md` folders from pinned upstream | `clarify-and-plan` for process ambiguity, frontend stack after decisions are clear | Do not treat as generic implementation planning or visual polish; load `layers-intro` first |
| Convert to Markdown | Local document conversion before reading PDFs, Office files, spreadsheets, HTML exports, CSV/data, or knowledge-vault docs | Yes; original local skill | ovd-workflow knowledge vault, `defuddle` for web pages | Uses MarkItDown when available; cannot intercept native model file ingestion and must not overwrite originals |
| Reddit Research | Public Reddit community signal, subreddit search, thread/comment analysis, and sentiment synthesis | Yes; original local skill | `last30days`, customer-research, content strategy | Best-effort only; no auth, no cookies, low volume, respect rate limits/blocks |
...
| Claude security-guidance | Claude Code official plugin/hook layer for generated-code security warnings, diff/commit reviews, and `claude-security-guidance.md` project policy | Claude Code only | Native `/security-review`, project-specific security rules | Do not present as portable; it may call model endpoints and should not receive secrets in guidance files |
