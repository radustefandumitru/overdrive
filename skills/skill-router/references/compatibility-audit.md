...
| Anthropic example skills | Official examples for brand guidelines, doc co-authoring, MCP server building, and Slack GIF creation | Yes; installed from pinned `anthropics/skills` paths | `doc-coauthoring` with GSD docs/spec work; `mcp-builder` with Context7/current MCP docs; `slack-gif-creator` with local image/GIF workflows | Use `brand-guidelines` only for explicitly Anthropic-branded artifacts; approval-gate Slack posting or live MCP publishing |
...
| Playwright | Browser validation, screenshots, snapshots, UI flows, data extraction | Yes where CLI/runtime is installed | Design stack, GSD verification, checkout/debug flows | Prefer `playwright-cli`; use OpenAI `playwright` wrapper as fallback/complement. Do not use for static code-only tasks unless real browser proof matters |
