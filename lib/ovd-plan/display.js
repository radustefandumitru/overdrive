...
function renderTreeBody(tree, opts) {
  ...
  function walk(node) {
    for (const child of node.children || []) {
      ...
      const insertedReason = isAgentInserted(child)
        ? (child.annotations && child.annotations.inserted_reason) || '(reason missing)'
        : null;
      const agentTag = insertedReason !== null ? ` [proposed-by-agent: ${insertedReason}]` : '';
