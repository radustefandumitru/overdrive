...
// Best-effort derive of the just-closed node when a caller doesn't pass one:
// the active node, if it is itself now closed (done/skipped). Used for bare
// /ovd-go-style checks; /ovd-log callers pass `justClosed` explicitly because
// the active marker has usually already advanced to the next leaf.
function deriveJustClosedFromState(rootDir) {
  const opened = openState(rootDir);
  if (!opened.ok) return null;
  const active = flattenNodes(opened.parsed.tree).find((n) => n.active);
  if (active && isNodeClosed(active)) return active.id;
  return null;
}

// Task 5.5 — the SHARED recursive-close-check entry both /ovd-log DEFAULT and
// HANDOFF call (and re-usable by /ovd-go). Guarantees identical walks regardless
// of caller: it is a thin wrapper over recursiveCloseFlow that centralizes the
// no-closure result shape and the derive-from-state fallback. No fork (FM #2).
function runRecursiveCloseCheck(rootDir, opts = {}) {
  if (!rootDir || typeof rootDir !== 'string') {
    return { ok: false, status: STATUS, reason: 'invalid-project-dir', text: 'runRecursiveCloseCheck requires a project directory string.' };
  }
  const explicit = (opts.justClosed != null) ? String(opts.justClosed).trim() : '';
  const justClosed = explicit || deriveJustClosedFromState(rootDir);
  if (!justClosed) {
    return { ok: true, status: STATUS, mode: 'no-closure', reason: 'nothing-closed', closures: [], stops_at: null, text: 'No leaf closed — nothing to close up.' };
  }
  return recursiveCloseFlow(rootDir, justClosed, opts);
}
