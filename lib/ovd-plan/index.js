...
const logDefaultModule = require('./log-default');
const docUpdateModule = require('./doc-update');
const concernsModule = require('./concerns');
const commitModule = require('./commit');
const milestoneCloseModule = require('./milestone-close');
const handoffModule = require('./handoff');
...
function runPlan(options = {}, env = process.env) {
  ...
  if (options.subcommand === 'intent') {
    ...
    return intentModule.runIntent(rootDir, {
      mode: isCommit ? 'commit' : 'plan',
      text: options.text || null,
      entries
    });
...
function runLog(options = {}, env = process.env) {
  ...
  // `/ovd-log handoff` (Task 5.2) — full 11-step end-of-session pipeline. PLAN
  // lists the steps; COMMIT executes them in order (Pattern-1, Pattern-4 guard).
  if (options.subcommand === 'handoff') {
    ...
    if (parseError) {
      return { ok: false, status: 'log', mode: 'commit', reason: `--entries-json was not valid JSON: ${parseError}`, text: `Invalid --entries-json: ${parseError}` };
    }
    return handoffModule.runHandoff(rootDir, { mode: isCommit ? 'commit' : 'plan', entries });
  }
  // `/ovd-log concerns [<node-ref>]` (Task 5.4) — structured 7-dimension review
  // on the active node (or specified). PLAN emits dimensions; COMMIT persists
  // findings to the node's `concerns` annotation (Pattern-1, Pattern-4 guard).
  if (options.subcommand === 'concerns') {
    const { resolveProjectDir } = require('../ovd-workflow');
    const rootDir = resolveProjectDir(options.projectDir, env);
    const isCommit = options.step === 'commit' || !!options.entriesJson;
    let entries = null;
    ...
    return concernsModule.runLogConcerns(rootDir, { mode: isCommit ? 'commit' : 'plan', nodeId: options.text || null, entries });
  }
  // `/ovd-log milestone-close <id>` (Task 5.6) — the close cascade. PLAN computes
  // signals + asks for learnings; COMMIT persists the summary + archives the
  // subtree verbatim. Consumed by HANDOFF step 7 (Pattern-1, Pattern-4 guard).
  if (options.subcommand === 'milestone-close') {
    const { resolveProjectDir } = require('../ovd-workflow');
    const rootDir = resolveProjectDir(options.projectDir, env);
    const isCommit = options.step === 'commit' || !!options.entriesJson;
    let entries = null;
    ...
    return milestoneCloseModule.runMilestoneClose(rootDir, { mode: isCommit ? 'commit' : 'plan', milestoneId: options.text, entries });
  }
  // `/ovd-log commit` (Task 5.8) — user-gated commit. PLAN proposes; only
  // `--confirm` (set after the user picks "commit") executes git. Never
  // auto-commits, never --no-verify. Primarily consumed by HANDOFF step 11.
  if (options.subcommand === 'commit') {
    const { resolveProjectDir } = require('../ovd-workflow');
    const rootDir = resolveProjectDir(options.projectDir, env);
    return commitModule.runCommit(rootDir, {
      mode: options.confirm ? 'commit' : 'plan',
      message: options.message,
      summary: options.text || options.summary,
      files: options.files,
  ...
  // `/ovd-log doc-update` (Task 5.7) — internal surgical doc propagation. PLAN
  // from change context; COMMIT on `--entries-json` (Pattern-1 dispatch, Pattern-4
  // guard). DEFAULT/HANDOFF/EDIT consume runDocUpdate; this exposes it for the
  // agent's plan→commit round-trip. `--confirm` applies non-trivial changes.
  if (options.subcommand === 'doc-update') {
    const { resolveProjectDir } = require('../ovd-workflow');
    const rootDir = resolveProjectDir(options.projectDir, env);
    const isCommit = options.step === 'commit' || !!options.entriesJson;
    let entries = null;
    ...
    if (parseError) {
      return { ok: false, status: 'doc-update', mode: 'commit', reason: `--entries-json was not valid JSON: ${parseError}`, text: `Invalid --entries-json: ${parseError}` };
    }
    return docUpdateModule.runDocUpdate(rootDir, { mode: isCommit ? 'commit' : 'plan', entries, confirm: !!options.confirm });
  }
  // Bare `/ovd-log` → DEFAULT lightweight save (Task 5.1). PLAN with no payload;
  // COMMIT on `--entries-json` (Pattern-1 dispatch, Pattern-4 JSON guard).
  if (!options.subcommand) {
    const { resolveProjectDir } = require('../ovd-workflow');
    const rootDir = resolveProjectDir(options.projectDir, env);
    const isCommit = options.step === 'commit' || !!options.entriesJson;
    ...
    return logDefaultModule.runLogDefault(rootDir, { mode: isCommit ? 'commit' : 'plan', entries });
  }
...
function runWorkflow(options = {}, env = process.env) {
  ...
  if (options.subcommand === 'knowledge') {
    // Task 2.9 — thin v2 umbrella over the preserved v1 knowledge-vault reindex
    // (legacy /ovd-knowledge delegates here). No v2-native replacement exists,
    // so the internals stay v1 (bounded "preserved as such" case).
    const ovdWorkflow = require('../ovd-workflow');
    const result = ovdWorkflow.knowledge({
    ...
    const text = result.apply
      ? `ovd-workflow knowledge vault reindexed ${result.trackedKnowledge || 0} file(s); ${changed.length} changed.`
      : `ovd-workflow knowledge dry-run: ${result.trackedKnowledge || 0} file(s); ${changed.length} changed. Re-run with --apply to write knowledge-index.json.`;
    return { ...result, text };
  }
...
module.exports = {
  ...
  logDefault: logDefaultModule,
  runLogDefault: logDefaultModule.runLogDefault,
  docUpdate: docUpdateModule,
  runDocUpdate: docUpdateModule.runDocUpdate,
  concerns: concernsModule,
  runLogConcerns: concernsModule.runLogConcerns,
  commit: commitModule,
  runCommit: commitModule.runCommit,
  milestoneClose: milestoneCloseModule,
  runMilestoneClose: milestoneCloseModule.runMilestoneClose,
  handoff: handoffModule,
  runHandoff: handoffModule.runHandoff,
  intent: intentModule,
  classifyIntent: intentModule.classifyIntent,
  renderAmbiguityPrompt: intentModule.renderAmbiguityPrompt,
  routeOrPrompt: intentModule.routeOrPrompt,
  recordIntentCorrection: intentModule.recordIntentCorrection,
  runIntent: intentModule.runIntent,
  routeBarePlan,
  emitEmptyPlanRoute
};
