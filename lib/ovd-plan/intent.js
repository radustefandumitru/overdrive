...
const { ensureSessionFile, formatStamp } = require('./log-capture');
...
const INTENT_CORRECTIONS_HEADER = 'intent-corrections';
...
// Task 6.4 — log a mis-classification (user correction) to the current session
// capture under a `## intent-corrections` section (Q6.6: session capture only).
// Reuses the canonical session-file primitive (`ensureSessionFile`) and section
// primitive (`appendUnderHeader`) — no fork (Pattern 2). Logging only; the
// classifier never auto-retunes (Q6.10). Aggregated at LEARNINGS EXTRACT.
function recordIntentCorrection(rootDir, entry, opts = {}) {
  if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
    return envelope({ ok: false, mode: 'commit', action: 'correction', reason: 'invalid-shape', text: 'Correction requires { originalMessage, classifiedAs, correctedTo }.' });
  }
  const originalMessage = oneLine(entry.originalMessage);
  const classifiedAs = oneLine(entry.classifiedAs);
  const correctedTo = oneLine(entry.correctedTo);
  const missing = [];
  if (!originalMessage) missing.push('originalMessage');
  if (!classifiedAs) missing.push('classifiedAs');
  if (!correctedTo) missing.push('correctedTo');
  if (missing.length) {
    return envelope({ ok: false, mode: 'commit', action: 'correction', reason: 'missing-fields', errors: missing, text: `Correction missing required field(s): ${missing.join(', ')}.` });
  }

  const { display } = formatStamp(nowIso(opts));
  const bullet = `- [${display}] classified "${originalMessage}" as \`${classifiedAs}\` → corrected to \`${correctedTo}\``;
  const summary = `intent mis-classification: "${originalMessage}" classified as ${classifiedAs}, corrected to ${correctedTo}`;

  const session = ensureSessionFile(rootDir, opts);
  let content;
  try {
    content = fs.readFileSync(session.file, 'utf8');
  } catch (err) {
    return envelope({ ok: false, mode: 'commit', action: 'correction', reason: 'read-error', text: `Could not read session file: ${err.message}` });
  }
  const updated = appendUnderHeader(content, INTENT_CORRECTIONS_HEADER, bullet);
  fs.writeFileSync(session.file, updated);
  ...
  return envelope({
    ...
    header: INTENT_CORRECTIONS_HEADER,
    // Pattern 6 — expose both summary and note so a `sub.summary || sub.note`
    // log-push consumer (workflow.js) degrades gracefully.
    summary,
    note: summary,
    text: `Logged mis-classification to ${session.rel} under "## ${INTENT_CORRECTIONS_HEADER}".`
  });
}
...
function runIntent(rootDir, opts = {}) {
  ...
  // Correction path (Task 6.4) — independent of the classify-message guard; the
  // original message rides the entries payload.
  if (isCommit && opts.entries && opts.entries.action === 'correction') {
    return recordIntentCorrection(rootDir, {
      originalMessage: opts.entries.original_message,
      classifiedAs: opts.entries.classified_as,
      correctedTo: opts.entries.corrected_to
    }, opts);
  }
