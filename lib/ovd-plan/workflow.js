...
function runMigrateLegacy(rootDir, opts = {}) {
  return {
    ...
    stub: true,
    mode: opts.mode || 'full',
    note: 'Task 2.2.5 placeholder — the real MIGRATE state (with per-file migration + archive) lands in the next commit.'
  };
}
...
function runPreferencesElicit(rootDir, opts = {}) {
  return {
    ok: true,
    status: 'preferences-elicit-stub',
    stub: true,
    note: 'Task 2.4 placeholder — Socratic preferences flow lands in a future commit.'
  };
}
...
function runWorkflowInit(rootDir, opts = {}) {
  ...
  if (step === 'migration') {
    if (action === 'migrate' || action === 'skip-migration') {
      ...
      log.push({ step: 'migrate', stub: migration.stub === true, mode: migration.mode, note: migration.note });
      const scaffold = ensureScaffolded(resolvedRoot);
      log.push({ step: 'scaffold', skipped: !scaffold.scaffolded, note: scaffold.action });
      const refreshed = inspectProject(resolvedRoot);
      return emitInitResult('mapping', refreshed, log, { prompt: mappingPrompt() });
  ...
  if (canonical[step]) {
    ...
    if (action === 'proceed') {
      ...
      log.push({ step: cfg.logKey, stub: sub.stub === true, note: sub.note });
      const refreshed = inspectProject(resolvedRoot);
