...
function buildPlanSkillsTurn(rootDir, opts = {}) {
  ...
  if (pending.length === 0) {
    return envelope({
      ...
      text: 'No leaves with pending_skill_resolution=true. The RESOLVE SKILLS sub-step is complete; next /ovd-plan deliberate should be Stage 7 (Present).'
    });
  }
  ...
  if (remainingIds.length > 1) {
    ...
    lines.push('This is the last pending leaf. Commit will transition stage → present (Stage 7).');
  }
...
function applyPlanSkillsTurn(rootDir, entries, opts = {}) {
  ...
  // transition to 'present'. Structural enforcement — agent can't skip leaves.
  ...
  const transitionTail = transitioned
    ? ' All leaves resolved. Transitioning to Stage 7 (Present + iterate).'
