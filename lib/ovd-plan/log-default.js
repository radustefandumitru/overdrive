...
// Sentinel lock (Q5.9) lives in ./log-lock — the SAME lock HANDOFF acquires, so
// concurrent /ovd-log invocations cannot interleave state writes (no fork).
...
function buildLogDefaultPlan(rootDir, opts = {}) {
  ...
  lines.push('    },');
  lines.push('    "docs": { "updates": [{ "doc": "<rel path>", "heading": "<section>", "body": "<new section content>" }] },');
  ...
  lines.push('Include "docs" only if a change affects documented surfaces (surgical propagation via doc-coauthoring); omit otherwise.');
...
function normalizeLogDefaultEntries(entries) {
  ...
  return {
    ...
    docs: (entries.docs && typeof entries.docs === 'object' && !Array.isArray(entries.docs)) ? entries.docs : null,
    closed_leaf
  };
}
...
function applyLogDefault(rootDir, entries, opts = {}) {
  ...
  try {
    ...
    // 4. DOC UPDATE — surgical propagation (5.7), honoring its action-path
    // threshold. Lightweight by default: runs only when the agent proposes doc
    // changes (a change affecting documented surfaces); otherwise a no-op.
    ...
    // 5. RECURSIVE CLOSE CHECK — the shared Task 5.5 entry both DEFAULT and
    // HANDOFF call (closure.js, no fork). DEFAULT passes the just-closed leaf
    // explicitly since the active marker has usually already advanced.
    const justClosed = deriveJustClosed(norm.closed_leaf, norm.state.status_changes);
    const closeFlow = runRecursiveCloseCheck(rootDir, { justClosed: justClosed || undefined });
