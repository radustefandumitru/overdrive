...
// Phase 6 Task 6.4: count logged intent mis-classifications across session files
// (entries written by intent.recordIntentCorrection under `## intent-corrections`).
// Surfaced as a LEARNINGS signal so recurring routing mistakes can be reviewed
// (no auto-retune — Q6.10). Mirrors countSkillDeltaMentions (Pattern 2 shape).
function countIntentCorrections(rootDir) {
  let count = 0;
  const dir = ovdPath(rootDir, 'sessions');
  let files;
  try { files = fs.readdirSync(dir); } catch (_) { return 0; }
  for (const f of files) {
    ...
    try {
      const content = fs.readFileSync(path.join(dir, f), 'utf8');
      count += (content.match(/→ corrected to /g) || []).length;
    } catch (_) { /* skip */ }
  }
  return count;
}
...
function gatherSignals(rootDir, milestone) {
  ...
  return {
    ...
    intentCorrections: countIntentCorrections(rootDir),
...
function buildMilestoneClosePlan(rootDir, opts = {}) {
  ...
  lines.push(`  - Intent mis-classifications logged: ${signals.intentCorrections}`);
  if (signals.intentCorrections > 0) {
    lines.push('    (review .overdrive/sessions/* "## intent-corrections" for recurring routing mistakes)');
  }
  lines.push('');
