...
const { acquireLogLock, releaseLogLock, lockedResult } = require('./log-lock');
...
function applyHandoff(rootDir, entries, opts = {}) {
  ...
  // Q5.9 — guard the state-writing critical section (steps 2–11 mutate OVERDRIVE.md
  // / handoffs / archive) with the SAME sentinel lock DEFAULT uses, so a concurrent
  // /ovd-log cannot interleave writes (no fork — the lock lives in ./log-lock).
  const lock = acquireLogLock(rootDir);
  if (!lock.ok) return lockedResult();
  try {
    return runHandoffSteps(rootDir, norm, opts);
  } finally {
    releaseLogLock(lock);
  }
}

function runHandoffSteps(rootDir, norm, opts) {
  ...
  // Step 11 — COMMIT (always offered, user-gated; 5.8). Proposes by default;
  // executes only when the handoff payload's commit.confirm is set (= user
  // approval). Message context = milestone-close if the cascade ran, else handoff.
  const milestoneClosed = !!(milestone_close && milestone_close.ok);
  const commitOpts = norm.commit || {};
  const commitFiles = ['OVERDRIVE.md', handoff_file];
  if (milestoneClosed && milestone_close.report) commitFiles.push(milestone_close.report);
  const commit = runCommit(rootDir, {
    mode: commitOpts.confirm ? 'commit' : 'plan',
    message: commitOpts.message,
    summary: commitOpts.summary || norm.summary.highlights[0] || 'session handoff',
    context: milestoneClosed ? 'milestone-close' : 'handoff',
    phase: commitOpts.phase,
    milestone: milestoneClosed ? milestone_close.milestone_id : undefined,
    files: commitFiles,
    confirm: !!commitOpts.confirm,
    runGit: opts.runGit
  });
  if (commit.mode === 'commit-done') completed.push(11);
  ...
  if (milestoneClosed) {
    ...
    out.push('');
  } else if (closeFlow.mode === 'closure-prompt' || closeFlow.mode === 'project-complete') {
  ...
  if (commit.mode === 'commit-done') {
    out.push(`Committed: ${commit.message}.`);
  } else {
    out.push('Step 11 — commit (user approval required):');
  ...
  return {
    ...
    milestone_close,
    commit,
    next_step: completed.includes(11) ? null : (completed.includes(7) ? 11 : 6),
    text: out.join('\n')
  };
}
