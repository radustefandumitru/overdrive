...
function readCalibration(rootDir) {
  ...
  const cal = innerObj[CALIBRATION_FIELD];
...
function buildCalibrationPlan(rootDir, opts = {}) {
  ...
  if (!opened.ok && opened.reason === 'missing-plan') {
    return {
      ok: false,
      status: 'calibrate',
      reason: 'missing-plan',
...
function applyCalibration(rootDir, calibrationInput, opts = {}) {
  ...
  const opened = openState(rootDir);
  if (!opened.ok) {
    return { ok: false, status: 'calibrate', reason: opened.reason, text: opened.text };
  }

  const prior = (opened.innerObj[CALIBRATION_FIELD]
    && typeof opened.innerObj[CALIBRATION_FIELD] === 'object'
    && !Array.isArray(opened.innerObj[CALIBRATION_FIELD]))
    ? opened.innerObj[CALIBRATION_FIELD]
    : null;
  const merged = mergeCalibration(prior, calibrationInput, now);

  opened.innerObj[CALIBRATION_FIELD] = merged;

  const committed = commitState(rootDir, opened);
  if (!committed.ok) {
    return { ok: false, status: 'calibrate', reason: committed.reason, text: committed.text };
  }
