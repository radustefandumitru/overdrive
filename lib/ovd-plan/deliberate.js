...
const STAGES = ['elicit', 'spec', 'plan', 'present', 'commit', 'committed'];
...
function buildSpecTurn(rootDir, opts = {}) {
  ...
  lines.push('');
  lines.push('Set "transition":"plan" to advance to Stage 5; omit to iterate Spec (e.g. re-elicit on high-ambiguity milestones).');
  return envelope('deliberate', {
    ok: true,
    mode: 'plan',
    stage: 'spec',
    answeredQuestions: answers,
    expectedPayload: { milestones: 'array', transition: 'null|"plan"' },
    text: lines.join('\n')
  });
}
...
function applySpecTurn(rootDir, entries, opts = {}) {
  ...
  if (entries.transition !== undefined && entries.transition !== null && entries.transition !== 'plan') {
    return envelope('deliberate', { ok: false, reason: 'invalid-transition', text: 'Spec transition must be null or "plan".' });
  }
  const validated = [];
  const highAmbiguity = [];
  ...
  const now = nowIso(opts);
  const inner = opened.innerObj;
  inner.stage = entries.transition === 'plan' ? 'plan' : 'spec';
  ...
  return envelope('deliberate', {
    ...
    mode: 'commit',
    stage: inner.stage,
    transitioned: entries.transition === 'plan',
    milestonesWritten: validated.length,
    highAmbiguity,
    text: `Spec recorded. ${validated.length} milestone(s) written to proposed_tree.${entries.transition === 'plan' ? ' Transitioning to Stage 5 (Plan).' : ''}${warnLines.join('\n')}`
  });
}
...
function buildLeafAnnotations(leaf) {
  const ann = {};
  // Writer key order: skills, confidence, rationale, considered, scope, success, deps, verify, references, cluster_verification
