...
const STAGES = ['elicit', 'spec', 'blind_spot', 'plan', 'plan_skills', 'verify', 'present', 'commit', 'committed'];
...
function buildPlanTurn(rootDir, opts = {}) {
  ...
  lines.push('');
  lines.push('Set "transition":"plan_skills" on the final commit (when all milestones have leaves) to advance to Stage 5.5 (RESOLVE SKILLS). Slice B fills skills/confidence/rationale/considered per leaf via the skill-router helper.');
  return envelope('deliberate', {
    ok: true,
    mode: 'plan',
    stage: 'plan',
    ...
    expectedPayload: { milestone_id: 'string', leaves: 'array', transition: 'null|"plan_skills"' },
    text: lines.join('\n')
  });
}
...
function applyPlanTurn(rootDir, entries, opts = {}) {
  ...
  if (entries.transition !== undefined && entries.transition !== null && entries.transition !== 'plan_skills') {
    return envelope('deliberate', { ok: false, reason: 'invalid-transition', text: 'Plan transition must be null or "plan_skills".' });
  }
  ...
  inner.stage = entries.transition === 'plan_skills' ? 'plan_skills' : 'plan';
  ...
  return envelope('deliberate', {
    ...
    mode: 'commit',
    stage: inner.stage,
    transitioned: entries.transition === 'plan_skills',
    milestoneId: entries.milestone_id,
    leavesWritten: annotated.length,
    milestonesRemaining: remaining.map((m) => m.id),
    text: `Plan recorded for milestone ${entries.milestone_id} (${annotated.length} leaf/leaves; pending_skill_resolution=true). ${remaining.length === 0 ? 'All milestones have leaves.' : `Remaining: ${remaining.map((m) => m.id).join(', ')}.`}${entries.transition === 'plan_skills' ? ' Transitioning to Stage 5.5 (RESOLVE SKILLS).' : ''}`
  });
}
...
function buildLeafAnnotations(leaf) {
  ...
  // Slice B (RESOLVE SKILLS) fills rationale + considered via the skill-router helper.
  // Copy through to annotations so Stage 8 commit round-trips them to OVERDRIVE.md.
  // writer.js ANNOTATION_KEY_ORDER places rationale + considered between confidence and scope.
  if (typeof leaf.rationale === 'string') ann.rationale = leaf.rationale;
  if (Array.isArray(leaf.considered)) ann.considered = leaf.considered;
  ann.scope = {
...
// ---------------------------------------------------------------------------
// Stage 6 — Verify (Slice C wrapper around plan-quality with auto-advance)
//
// The verify stage delegates the actual coverage / leaf-completeness / goal-
// backward audit to plan-quality.js (Task 3.8). This wrapper is the state-
// machine owner's responsibility — it translates the audit envelope into a
// deliberation-state transition: on a clean envelope (no uncovered functional
// requirements, zero failed leaves, every milestone verdict === 'pass'), the
// stage advances to 'present' (Stage 7). Otherwise the stage stays at 'verify'
// so the user can iterate (typically by re-running /ovd-plan deliberate which
// loops back to the offending earlier stage).
//
// Per the Q3.8.1 / Q3.8.11 boundary lock, plan-quality.js stays standalone —
// it does NOT know about deliberation-state stage transitions. The `/ovd-plan
// verify` retrospective subcommand calls runPlanQualityCheck directly (no
// wrapper, no transition), preserving the audit-without-stage semantics.
// ---------------------------------------------------------------------------

function runVerifyStage(rootDir, opts = {}) {
  const result = require('./plan-quality').runPlanQualityCheck(rootDir, opts);
  const isCommit = (opts && opts.mode === 'commit') || !!(opts && opts.entries);
  if (!isCommit || !result || !result.ok) return result;
  const coverageClean = result.coverage
    && Array.isArray(result.coverage.uncovered_requirement_indices)
    && result.coverage.uncovered_requirement_indices.length === 0;
  const leafClean = result.leaf_completeness
    && Array.isArray(result.leaf_completeness.failed)
    && result.leaf_completeness.failed.length === 0;
  const verdictsClean = Array.isArray(result.goal_backward)
    && result.goal_backward.length > 0
    && result.goal_backward.every((v) => v && v.verdict === 'pass');
  if (coverageClean && leafClean && verdictsClean) {
    const opened = openState(rootDir);
    if (opened.ok && currentStage(opened.innerObj) === 'verify') {
      opened.innerObj.stage = 'present';
      opened.innerObj.last_action = nowIso(opts);
      opened.innerObj = reorderInner(opened.innerObj);
      const committed = commitState(rootDir, opened);
      if (committed.ok) {
        return Object.assign({}, result, {
          stage: 'present',
          transitioned: true,
          text: (result.text || '') + '\n\nAudit clean — transitioning to Stage 7 (Present + iterate).'
        });
      }
    }
    return result;
  ...
  return Object.assign({}, result, {
    stage: 'verify',
    transitioned: false,
    text: (result.text || '') + '\n\nAudit has open items — stage stays at Stage 6 (Verify). Re-run /ovd-plan deliberate to revisit (or iterate via plan/spec).'
  });
}

function runDeliberate(rootDir, opts = {}) {
  // Task 3.9 — Multi-session re-entry intercept (r3 §5.7 + Q12). Runs BEFORE
  // the existing stage switch. Two-phase detection: stale-state (tree drift)
  // wins over time-threshold re-entry because a drifted tree makes any summary
  // potentially misleading. Once `pending_reentry: true` is set, commit-mode
  // routes through applyReentryTurn until cleared by an action commit.
  // The `opts.skipReentry` opt-out is for test use only — production callers
  // never set it.
  if (!opts.skipReentry) {
    const reentry = require('./reentry');
    const opened = openState(rootDir);
    if (opened.ok) {
      const isCommitEarly = opts.mode === 'commit' || !!opts.entries;
      if (isCommitEarly && opened.innerObj.pending_reentry === true) {
        return reentry.applyReentryTurn(rootDir, opts.entries, opts);
      }
      if (!isCommitEarly) {
        const stale = reentry.detectStaleState(opened);
        if (stale.drifted) {
          return reentry.buildStaleTurn(rootDir, stale, opts);
        }
        const re = reentry.detectReentry(opened, opts);
        if (re.needsReentry) {
          return reentry.buildReentryTurn(rootDir, opts);
        }
      }
    }
  }
  ...
  if (isCommit) {
    switch (stage) {
      ...
      case 'plan_skills':  return require('./plan-skills').runPlanSkills(rootDir, opts);
      case 'verify':       return runVerifyStage(rootDir, opts);
