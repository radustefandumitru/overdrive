  ...
  if (!fs.existsSync(root)) {
    if (fs.existsSync(legacyWorkflowPath(projectDir))) {
      return {
        ...
        projectDir,
        message: 'Legacy AgenticSupercharge workflow state exists. Run an Overdrive write action such as `overdrive checkpoint` or `overdrive resync --apply` to migrate it into .overdrive/.'
      };
    }
    return {
      ok: true,
...
function doctor(options = {}) {
  ...
  if (!fs.existsSync(root)) {
    if (fs.existsSync(legacyWorkflowPath(projectDir))) {
      return { ok: true, initialized: false, legacyAvailable: true, projectDir, issues: ['Legacy .agenticsupercharge/ exists; run an Overdrive write action to migrate it into .overdrive/.'] };
    }
