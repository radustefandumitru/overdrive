...
function isWorkflowDisabled(env = process.env) {
  return String(env.OVERDRIVE_WORKFLOW || '').toLowerCase() === 'disabled';
}

function resolveProjectDir(value, env = process.env) {
  const raw = value || env.OVERDRIVE_PROJECT_DIR || env.CLAUDE_PROJECT_DIR || env.GEMINI_PROJECT_DIR || env.GEMINI_CWD || env.PWD || process.cwd();
...
function doctor(options = {}) {
  ...
  if (!fs.existsSync(gitignore)) {
    ...
    for (const entry of [`${workflowDirName}/`]) {
