...
const workflowCommands = new Set(['status', 'resync', 'knowledge', 'doctor', 'checkpoint', 'migrate', 'hook', 'route', 'usage']);
...
function parseArgs(argv) {
  ...
  if (args[0] && !args[0].startsWith('-')) {
    ...
    if (['install', 'verify', 'check-updates', 'list-targets', 'self-update', 'update-skills', 'uninstall', 'status', 'resync', 'knowledge', 'doctor', 'checkpoint', 'migrate', 'hook', 'route', 'usage', 'help'].includes(first)) {
      command = first;
    } else {
...
async function runWorkflowCommand(command, options, env) {
  ...
  if (command === 'migrate') {
    const result = ovdWorkflow.migrate(workflowOptions);
    const text = result.migrated
      ? `${result.dryRun ? 'would migrate' : 'migrated'} ovd-workflow legacy state: ${result.from} -> ${result.to}`
      : `ovd-workflow migration skipped: ${result.reason}`;
    printWorkflowResult(result, options.json, text);
    return;
  }
  if (command === 'route') {
...
async function runInstall(ctx, options) {
  ...
  migrateCurrentProjectWorkflow(ctx, options, plan);
...
function installWorkflowRuntime(ctx, options, plan) {
  ...
  installLegacyRuntimeCompatibility(ctx, currentDir);
...
function installLegacyRuntimeCompatibility(ctx, currentDir) {
  const legacyCurrentDir = path.join(legacyWorkflowBaseDir(ctx), 'runtime', 'current');
  fs.mkdirSync(path.dirname(legacyCurrentDir), { recursive: true });
  fs.rmSync(legacyCurrentDir, { recursive: true, force: true });
  try {
    fs.symlinkSync(currentDir, legacyCurrentDir, 'dir');
  } catch (_error) {
    fs.cpSync(currentDir, legacyCurrentDir, { recursive: true, filter: shouldCopy });
  }
}
...
function migrateCurrentProjectWorkflow(ctx, options, plan) {
  const projectDir = path.resolve(plan.scope === 'local' ? plan.projectDir : options.projectDir || process.cwd());
  const legacyRoot = ovdWorkflow.legacyWorkflowPath(projectDir);
  const root = ovdWorkflow.workflowPath(projectDir);
  if (!fs.existsSync(legacyRoot) || fs.existsSync(root)) return;
  const result = ovdWorkflow.migrate({
    projectDir,
  ...
  if (result.migrated) {
    log(`${options.dryRun ? 'would migrate' : 'migrated'} ovd-workflow legacy state -> ${root}`);
  } else {
    log(`skipped ovd-workflow migration for ${projectDir}: ${result.reason}`);
  }
}
...
function isWorkflowCommand(command) {
  ...
  return tokens.some((token) => {
    ...
    return base === 'overdrive'
      || base === 'overdrive.js'
      || base === 'ovd'
      || base === 'agentic-supercharge'
      || base === 'agentic-supercharge.js';
  });
}
...
Usage:
  ...
  overdrive migrate [--project-dir PATH] [--apply] [--json]
