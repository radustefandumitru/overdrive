...
const legacyManagedBlockStart = '<!-- ai-skill-setup:global-guidelines:start -->';
const legacyManagedBlockEnd = '<!-- ai-skill-setup:global-guidelines:end -->';
const markerFile = '.overdrive.json';
const legacyMarkerFile = '.agentic-supercharge.json';
const backupRootName = '.overdrive';
const legacyBackupRootName = '.agentic-supercharge';
const managedTextNamespace = 'overdrive:managed';
const legacyManagedTextNamespace = 'agentic-supercharge:managed';
...
async function runWorkflowCommand(command, options, env) {
  ...
  if (command === 'doctor') {
    ...
    printWorkflowResult(result, options.json, result.issues.length ? `ovd-workflow doctor found ${result.issues.length} issue(s):\n- ${result.issues.join('\n- ')}` : 'ovd-workflow doctor: clean.');
...
function createContext(options, env = process.env) {
  const kitDir = env.OVERDRIVE_KIT_DIR || env.AGENTIC_SUPERCHARGE_KIT_DIR || env.AI_SKILL_KIT_DIR || path.resolve(__dirname, '..');
...
async function runUninstall(ctx, options) {
  ...
  console.log('Only skill folders containing .overdrive.json or legacy .agentic-supercharge.json markers will be removed.');
...
function patchClaudeVideoSetupScript(src, skillName, sourceId) {
  ...
  text = text
    ...
    .replace(/- Never sudo\. On macOS, auto-install via brew\. Elsewhere, print exact commands\./, '- Never sudo or install packages from this runtime checker. The Overdrive installer handles optional setup; otherwise print exact commands for the user to run.');
...
function uninstallMarkedSkills(ctx, options, plan) {
  ...
  if (!count) log('No Overdrive-marked or legacy managed skill folders found in selected roots.');
}
...
function installWorkflowRuntime(ctx, options, plan) {
  ...
  const legacyShim = path.join(legacyWorkflowBaseDir(ctx), 'bin', 'agentic-supercharge');
  if (options.dryRun) {
    ...
    log(`would install legacy CLI alias -> ${legacyShim}`);
    return;
  }
  ...
  writeWorkflowShim(legacyShim, currentDir, 'legacy');
...
function writeWorkflowShim(shimPath, currentDir, mode = 'canonical') {
  fs.mkdirSync(path.dirname(shimPath), { recursive: true });
  const label = mode === 'legacy' ? 'Overdrive managed legacy CLI shim' : 'Overdrive managed CLI shim';
...
function uninstallWorkflowRuntime(ctx, options) {
  ...
  const legacyBase = legacyWorkflowBaseDir(ctx);
  const legacyShim = path.join(legacyBase, 'bin', 'agentic-supercharge');
  if (options.dryRun) {
    ...
    log(`would remove legacy managed runtime versions under -> ${path.join(legacyBase, 'runtime')}`);
    log(`would remove legacy managed CLI shim -> ${legacyShim}`);
    return;
  }
  ...
  if (fs.existsSync(legacyShim) && /(?:Overdrive|AgenticSupercharge) managed (?:legacy )?CLI shim/.test(fs.readFileSync(legacyShim, 'utf8'))) {
    fs.rmSync(legacyShim, { force: true });
  }
  removeManagedRuntimeRoot(path.join(base, 'runtime'));
  removeManagedRuntimeRoot(path.join(legacyBase, 'runtime'));
...
function installWorkflowRule(ctx, options, targetKey, workflow) {
  ...
  const legacyDest = path.join(dir, 'agentic-supercharge-workflow.mdc');
...
`);
  if (options.dryRun) {
    ...
    if (fs.existsSync(legacyDest)) log(`would remove managed legacy workflow rule if marked -> ${legacyDest}`);
    return;
  }
  if (fs.existsSync(legacyDest)) removeManagedTextFile(legacyDest, `removed legacy workflow rule -> ${legacyDest}`);
  writeManagedTextFile(dest, body, `ovd-workflow rule for ${targetKey}`);
}

function uninstallWorkflowRule(ctx, options, workflow) {
  const dir = resolvePath(workflow.rulesDir, ctx);
  for (const name of ['overdrive-workflow.mdc', 'agentic-supercharge-workflow.mdc']) {
...
function legacyWorkflowBaseDir(ctx) {
  return path.join(ctx.home, legacyBackupRootName);
}
...
function isWorkflowHook(hook) {
  return Boolean(hook?.command && isWorkflowCommand(hook.command)) || /^(overdrive|agentic-supercharge)-/.test(hook?.name || '');
}

function isWorkflowCommand(command) {
  const text = String(command || '');
  return /(?:overdrive|agentic-supercharge)(?:\.js)?(?:\s|")/.test(text) && text.includes('hook');
}

function workflowCommandBodies(ctx, targetKey) {
  const shim = workflowShimPath(ctx);
  const canonical = {
    'ovd-status': `Run \`${shim} status --project-dir "$PWD"\` and summarize the current ovd-workflow state briefly.`,
    'ovd-resync': `Run \`${shim} resync --project-dir "$PWD" --dry-run\`. If the user confirms, rerun with \`--apply\`.`,
    'ovd-knowledge': `Run \`${shim} knowledge --project-dir "$PWD" --dry-run\`. If the user confirms, rerun with \`--apply\` to refresh the local knowledge vault index.`,
    'ovd-doctor': `Run \`${shim} doctor --project-dir "$PWD"\` and explain any workflow issues without changing project code.`,
    'ovd-checkpoint': `Run \`${shim} checkpoint --project-dir "$PWD"\` to create a local handoff snapshot, then report the created file.`,
  ...
  return {
    ...
    'as-status': managedText('ovd-workflow legacy alias as-status', `Legacy alias for /ovd-status. ${canonical['ovd-status']}`),
    'as-resync': managedText('ovd-workflow legacy alias as-resync', `Legacy alias for /ovd-resync. ${canonical['ovd-resync']}`),
    'as-knowledge': managedText('ovd-workflow legacy alias as-knowledge', `Legacy alias for /ovd-knowledge. ${canonical['ovd-knowledge']}`),
    'as-doctor': managedText('ovd-workflow legacy alias as-doctor', `Legacy alias for /ovd-doctor. ${canonical['ovd-doctor']}`),
    'as-checkpoint': managedText('ovd-workflow legacy alias as-checkpoint', `Legacy alias for /ovd-checkpoint. ${canonical['ovd-checkpoint']}`),
    'as-usage': managedText('ovd-workflow legacy alias as-usage', `Legacy alias for /ovd-usage. ${canonical['ovd-usage']}`)
  };
}
...
function hasManagedTextMarker(text) {
  return text.includes(managedTextNamespace) || text.includes(legacyManagedTextNamespace);
}
...
function shouldCopy(src) {
  ...
  if (base === markerFile || base === legacyMarkerFile) return false;
  if (base === '.overdrive' || base === '.agenticsupercharge') return false;
...
function hasMarker(dir) {
  return fs.existsSync(path.join(dir, markerFile)) || fs.existsSync(path.join(dir, legacyMarkerFile));
}

function readMarker(dir) {
  for (const name of [markerFile, legacyMarkerFile]) {
...
function upsertManagedBlock(currentText, blockText, targetPath) {
  ...
  const legacyPattern = new RegExp(`${escapeRegExp(legacyManagedBlockStart)}[\\s\\S]*?${escapeRegExp(legacyManagedBlockEnd)}\\n?`);
  assertManagedMarkerPair(currentText, targetPath);
  if (pattern.test(currentText)) return currentText.replace(pattern, block);
  if (legacyPattern.test(currentText)) return currentText.replace(legacyPattern, block);
...
function removeManagedBlock(currentText, targetPath) {
  assertManagedMarkerPair(currentText, targetPath);
  const pattern = new RegExp(`\\n?(?:${escapeRegExp(managedBlockStart)}[\\s\\S]*?${escapeRegExp(managedBlockEnd)}|${escapeRegExp(legacyManagedBlockStart)}[\\s\\S]*?${escapeRegExp(legacyManagedBlockEnd)})\\n?`, 'g');
...
function assertManagedMarkerPair(text, targetPath) {
  ...
  const legacyStarts = (text.match(new RegExp(escapeRegExp(legacyManagedBlockStart), 'g')) || []).length;
  const legacyEnds = (text.match(new RegExp(escapeRegExp(legacyManagedBlockEnd), 'g')) || []).length;
  if (starts !== ends) {
  ...
  if (legacyStarts !== legacyEnds) {
    throw new Error(`Instruction file ${targetPath} has mismatched legacy ai-skill-setup managed block markers (${legacyStarts} start, ${legacyEnds} end). Restore or remove the orphan marker before running this command.`);
  }
  if (starts + legacyStarts > 1) {
    throw new Error(`Instruction file ${targetPath} has ${starts + legacyStarts} managed blocks. Keep one block or remove duplicates before running this command.`);
  }
}
