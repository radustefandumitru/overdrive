...
const workflowCommands = new Set(['status', 'resync', 'knowledge', 'doctor', 'checkpoint', 'hook', 'route']);
...
function parseArgs(argv) {
  ...
  if (args[0] && !args[0].startsWith('-')) {
    ...
    if (['install', 'verify', 'check-updates', 'list-targets', 'self-update', 'update-skills', 'uninstall', 'status', 'resync', 'knowledge', 'doctor', 'checkpoint', 'hook', 'route', 'help'].includes(first)) {
      command = first;
    } else {
...
function uninstallWorkflowCommands(ctx, options, workflow) {
  const dir = resolvePath(workflow.commandsDir, ctx);
  for (const name of ['as-status', 'as-resync', 'as-knowledge', 'as-doctor', 'as-checkpoint']) {
    const dest = path.join(dir, `${name}.md`);
...
- Use the installed runtime command when available: \`${workflowShimPath(ctx)} status\`, \`${workflowShimPath(ctx)} doctor\`, \`${workflowShimPath(ctx)} knowledge\`, \`${workflowShimPath(ctx)} checkpoint\`.
...
function printWelcome(options, plan) {
  ...
  console.log('');
  console.log('Natural triggers: ask for "show status", "what should I consider?", "download this as mp3", "run React doctor", or any normal build/review task.');
