...
const asWorkflow = require('./as-workflow');
...
const publicRepoUrl = 'https://github.com/radustefandumitru/AgenticSupercharge.git';
...
const workflowCommands = new Set(['status', 'resync', 'knowledge', 'doctor', 'checkpoint', 'hook', 'route', 'usage']);
...
function parseArgs(argv) {
  ...
  if (args[0] && !args[0].startsWith('-')) {
    ...
    if (['install', 'verify', 'check-updates', 'list-targets', 'self-update', 'update-skills', 'uninstall', 'status', 'resync', 'knowledge', 'doctor', 'checkpoint', 'hook', 'route', 'usage', 'help'].includes(first)) {
      command = first;
    } else {
...
async function runWorkflowCommand(command, options, env) {
  ...
  if (command === 'status') {
    const result = asWorkflow.status(workflowOptions);
    printWorkflowResult(result, options.json, asWorkflow.formatStatus(result));
    return;
  }
  if (command === 'resync') {
    const result = asWorkflow.resync(workflowOptions);
    printWorkflowResult(result, options.json, result.apply
      ? `AS-Workflow resynced ${result.trackedFiles} file(s); ${result.changed.length} changed.`
      : `AS-Workflow resync dry-run: ${result.trackedFiles} file(s); ${result.changed.length} changed. Re-run with --apply to write file-index.json.`);
    return;
  }
  if (command === 'knowledge') {
    const result = asWorkflow.knowledge(workflowOptions);
    printWorkflowResult(result, options.json, result.apply
      ? `AS-Workflow knowledge vault reindexed ${result.trackedKnowledge || 0} file(s); ${result.changed.length} changed.`
      : `AS-Workflow knowledge dry-run: ${result.trackedKnowledge || 0} file(s); ${result.changed.length} changed. Re-run with --apply to write knowledge-index.json.`);
    return;
  }
  if (command === 'doctor') {
    const result = asWorkflow.doctor(workflowOptions);
    printWorkflowResult(result, options.json, result.issues.length ? `AS-Workflow doctor found ${result.issues.length} issue(s):\n- ${result.issues.join('\n- ')}` : 'AS-Workflow doctor: clean.');
    if (result.initialized && result.issues.length) process.exitCode = 1;
    return;
  }
  if (command === 'checkpoint') {
    const result = asWorkflow.checkpoint(workflowOptions);
    printWorkflowResult(result, options.json, `AS-Workflow checkpoint created: ${result.file}`);
    return;
  }
  if (command === 'route') {
    const result = asWorkflow.recordRoute(workflowOptions);
    printWorkflowResult(result, options.json, result.skipped ? `AS-Workflow route skipped: ${result.reason}` : 'AS-Workflow route recorded.');
    return;
  }
  if (command === 'usage') {
    const result = asWorkflow.usage(workflowOptions);
    printWorkflowResult(result, options.json, asWorkflow.formatUsage(result));
    return;
  }
  if (command === 'hook') {
    const stdin = await readStdin();
    const event = options.event || inferHookEvent(options);
    const result = asWorkflow.hook({ ...workflowOptions, event, stdin });
    if (event === 'statusline') {
      process.stdout.write(`${result.text || 'AS: off'}\n`);
    } else {
...
async function runUpdateSkills(ctx, options) {
  ...
  if (options.allSkills) {
    log(`Updating all matching AgenticSupercharge skills from ${options.allowUpstreamDrift ? 'tracking upstream sources' : 'verified pinned sources'}.`);
    log('Conflict policy: backup-and-replace. Existing matching folders are moved to ~/.agentic-supercharge/backups/... before replacement.');
  } else {
    log(`Updating AgenticSupercharge-managed skills from ${options.allowUpstreamDrift ? 'tracking upstream sources' : 'verified pinned sources'}.`);
...
async function runUninstall(ctx, options) {
  ...
  const plan = buildInstallPlan(ctx, options);
  console.log('\nAgenticSupercharge uninstall plan');
  console.log(`Scope: ${plan.scope}${plan.projectDir ? ` (${plan.projectDir})` : ''}`);
  console.log('Only skill folders containing .agentic-supercharge.json will be removed.');
  ...
  if (!options.dryRun && !options.yes && process.stdin.isTTY && process.stdout.isTTY) {
    ...
    try {
      const confirm = await askChoice(rl, 'Remove AgenticSupercharge-managed files only?', [
        ['no', 'No, exit'],
...
function runSelfUpdate(ctx, options) {
  ...
  if (status && !options.allowDirtySelfUpdate) {
    const message = 'Refusing to self-update because the AgenticSupercharge repo has uncommitted changes. Commit/stash them first, or rerun with --allow-dirty-self-update.';
    if (options.dryRun) {
      log(message);
      return;
    }
  ...
  if (options.dryRun) {
    log(`would update AgenticSupercharge kit in ${ctx.kitDir}`);
    log(`would run: git -C ${ctx.kitDir} pull --ff-only`);
    log(`current branch: ${branch}; upstream: ${upstream}`);
    return;
  }

  log(`Updating AgenticSupercharge kit from ${upstream}...`);
...
async function runCheckUpdates(ctx, options) {
  ...
  if (!result.checked) {
    console.log('Could not check for AgenticSupercharge updates right now. Try again later.');
    if (result.reason) console.log(`Reason: ${result.reason}`);
    return;
  }

  if (result.kitBehind) {
    console.log(`A newer AgenticSupercharge version is available (v${result.currentVersion} -> v${result.latestVersion}). Run ./update.sh to apply, or ./update.sh --kit-only to refresh just the installer.`);
  } else {
    console.log(`AgenticSupercharge v${result.currentVersion} is current.`);
  }

  if (result.skillUpdates.length) {
    const count = result.skillUpdates.reduce((sum, item) => sum + item.skillCount, 0);
    console.log(`${count} installed skill${count === 1 ? '' : 's'} from ${result.skillUpdates.length} upstream source${result.skillUpdates.length === 1 ? '' : 's'} have tracking-branch updates available. Update AgenticSupercharge when a release verifies newer pins, or run ./update.sh --skills-only --allow-upstream-drift if you intentionally want live upstream refs.`);
...
function maybePrintPassiveUpdateNotice(ctx, options) {
  ...
  try {
    ...
    if (result.kitBehind) {
      console.log(`Update available: AgenticSupercharge v${result.currentVersion} -> v${result.latestVersion}. Run ./update.sh or agentic-supercharge check-updates.`);
...
function checkManagedSkillUpdates(ctx, options) {
  ...
  for (const target of plan.skillTargets) {
    ...
    for (const entry of fs.readdirSync(target.skillRootAbs)) {
      const markerPath = path.join(target.skillRootAbs, entry, markerFile);
  ...
  for (const [root, expectedSkills] of expectedByRoot.entries()) {
    ...
    for (const skill of expectedSkills) {
      ...
      if (fs.existsSync(skillMd)) {
        ...
        if (skill === 'graphify') {
          check(`${skill} in ${root} uses normalized SKILL.md`, path.basename(skillMd) === 'SKILL.md');
          check(`${skill} in ${root} has AgenticSupercharge setup note`, skillText.includes('AgenticSupercharge attempts to set up Graphify during install') && skillText.includes('graphifyy==0.1.14') && skillText.includes('Python 3.10-3.12'));
...
async function askSkillSelection(rl, ctx, options) {
  ...
  log('   Recommended. Installs the full AgenticSupercharge library and lets skill-router choose only what each task needs.\n');
...
function collectSkillCatalog(ctx) {
  ...
  for (const skill of ctx.manifest.localSkills || []) {
    entries.push({ name: skill.to, group: 'AgenticSupercharge local skills', source: 'local-kit' });
  }
...
function printInstallSummary(ctx, options, plan) {
  console.log(`\n${style('AgenticSupercharge install plan', 'bold')}`);
  console.log(`Scope: ${plan.scope}${plan.projectDir ? ` (${plan.projectDir})` : ''}`);
  ...
  console.log(`Optional tool setup: ${options.noToolInstall ? 'skipped by --no-tool-install' : 'attempt safe non-privileged setup when selected skills need it'}`);
  ...
  if (!options.noToolInstall) {
    console.log('Use --no-tool-install to skip optional Graphify, video, and browser helper setup.');
  }
}
...
function prepareSkillSource(src, skillName, sourceId, sourceMeta = {}) {
  ...
  if (skillFile !== 'SKILL.md') {
    ...
    fs.copyFileSync(alternate, path.join(src, 'SKILL.md'));
  }
...
AgenticSupercharge attempts to set up Graphify during install using \`${graphifyPackageSpec}\` through pipx or a managed venv, preferring Python 3.10-3.12 for the pinned dependency tree. It never uses sudo, never uses global pip, and never hard-fails the main install if setup is unavailable.

\`\`\`bash
command -v graphify >/dev/null 2>&1 || "$HOME/.agentic-supercharge/bin/graphify" --help >/dev/null 2>&1 || python3 -c "import graphify" 2>/dev/null
\`\`\`
...
\`\`\`text
Graphify is not available yet. AgenticSupercharge normally attempts setup during install.
...
If a Graphify graph already exists for this project, prefer querying it before broad \`rg\` exploration for relationship/orientation questions. If the graph is stale, recommend Graphify's own \`--watch\` or git-hook workflow; AgenticSupercharge must not run Graphify in the background itself.

When Graphify is unavailable, continue with normal repository exploration (\`rg\`, file reads, existing AS-Workflow state) and be clear that the graph-backed path was skipped.
...
\`\`\`text
Design Extract browser support is normally prepared during AgenticSupercharge install.
...
Claude Video is optional in AgenticSupercharge. During install, AgenticSupercharge attempts safe non-privileged setup for ffmpeg/ffprobe and yt-dlp using Homebrew, winget, pipx, or a managed venv when available. It never uses sudo/elevation, never writes API keys, and never hard-fails the main install if optional video tooling cannot be installed.
...
\`\`\`text
Claude Video needs ffmpeg/ffprobe and yt-dlp for video frames and captions. AgenticSupercharge normally attempts setup during install.
...
`;
  ...
  text = next
    ...
    .replace(/auto-installs/gi, 'is prepared by AgenticSupercharge install when possible')
  ...
  fs.writeFileSync(skillPath, text);
  patchClaudeVideoSetupScript(src, skillName, sourceId);
}
...
function patchClaudeVideoSetupScript(src, skillName, sourceId) {
  const setupPath = path.join(src, 'scripts', 'setup.py');
  if (!fs.existsSync(setupPath)) return;
  let text = fs.readFileSync(setupPath, 'utf8');
  text = text
    .replace(/  setup\.py\s+Installer\. Auto-installs deps, scaffolds \.env, marks SETUP_COMPLETE\./, '  setup.py              Setup checker. Reports missing deps, never installs them automatically.')
    .replace(/- Never sudo\. On macOS, auto-install via brew\. Elsewhere, print exact commands\./, '- Never sudo or install packages from this runtime checker. The AgenticSupercharge installer handles optional setup; otherwise print exact commands for the user to run.');
  const macosHelper = `def _install_macos(missing: list[str]) -> tuple[bool, str]:
    pkgs = _brew_pkg(missing)
    if not pkgs:
        return True, "nothing to install"
    command = "brew install " + " ".join(pkgs)
    return False, "please install manually: " + command
`;
  text = text.replace(/def _install_macos\(missing: list\[str\]\) -> tuple\[bool, str\]:[\s\S]*?\n\ndef _install_hint_linux/, `${macosHelper}\ndef _install_hint_linux`);
  const replacement = `        if system == "Darwin":
            print("[setup] dependencies missing on macOS — please install:", file=sys.stderr)
            print("  brew install " + " ".join(_brew_pkg(missing)), file=sys.stderr)
            return 2
`;
  const next = text.replace(/        if system == "Darwin":[\s\S]*?        elif system == "Linux":/, `${replacement}        elif system == "Linux":`);
  if (next === text) throw new Error(`${sourceId}:${skillName} Claude Video setup transform could not patch macOS installer branch`);
  text = next;
  if (/subprocess\.run\(cmd\)/.test(text)) {
    throw new Error(`${sourceId}:${skillName} Claude Video setup transform left a brew auto-install call`);
  }
...
function applyHumanizerEthicsTransform(skillPath) {
  let text = fs.readFileSync(skillPath, 'utf8');
  if (text.includes('## AgenticSupercharge Ethics Note')) return;
  const note = `## AgenticSupercharge Ethics Note
...
function uninstallMarkedSkills(ctx, options, plan) {
  ...
  for (const target of plan.skillTargets) {
    ...
    for (const entry of fs.readdirSync(target.skillRootAbs)) {
      ...
      if (!fs.existsSync(path.join(skillDir, markerFile))) continue;
  ...
  if (!count) log('No AgenticSupercharge-marked skill folders found in selected roots.');
}
...
function installWorkflowRuntime(ctx, options, plan) {
  ...
  if (options.dryRun) {
    log(`would install AS-Workflow runtime -> ${runtimeDir}`);
    log(`would update AS-Workflow runtime pointer -> ${currentDir}`);
    log(`would install AS-Workflow CLI shim -> ${shim}`);
    return;
  }

  fs.rmSync(runtimeDir, { recursive: true, force: true });
  fs.mkdirSync(runtimeDir, { recursive: true });
  fs.cpSync(path.join(ctx.kitDir, 'bin'), path.join(runtimeDir, 'bin'), { recursive: true, filter: shouldCopy });
  fs.mkdirSync(path.join(runtimeDir, 'lib'), { recursive: true });
  for (const file of ['installer.js', 'as-workflow.js']) {
    fs.copyFileSync(path.join(ctx.kitDir, 'lib', file), path.join(runtimeDir, 'lib', file));
  }
  for (const file of ['package.json']) {
    fs.copyFileSync(path.join(ctx.kitDir, file), path.join(runtimeDir, file));
  }
  ...
  fs.writeFileSync(shim, `#!/bin/sh\n# AgenticSupercharge managed CLI shim\nexec node "${path.join(currentDir, 'bin/agentic-supercharge.js')}" "$@"\n`);
  fs.chmodSync(shim, 0o755);
  log(`installed AS-Workflow runtime -> ${runtimeDir}`);
  if (plan.scope === 'global') log('AS-Workflow hooks will use the persistent runtime path, so npx temp folders are not referenced.');
}
...
function uninstallWorkflowRuntime(ctx, options) {
  ...
  const shim = workflowShimPath(ctx);
  if (options.dryRun) {
    log(`would remove managed AS-Workflow runtime versions under -> ${path.join(base, 'runtime')}`);
    log(`would remove managed AS-Workflow CLI shim -> ${shim}`);
    return;
  }
  if (fs.existsSync(shim) && fs.readFileSync(shim, 'utf8').includes('AgenticSupercharge managed CLI shim')) {
  ...
  for (const entry of fs.readdirSync(runtimeRoot)) {
    ...
    } else if (fs.existsSync(path.join(full, markerFile))) {
      fs.rmSync(full, { recursive: true, force: true });
    }
  }
  log('removed managed AS-Workflow runtime files');
}
...
function installWorkflowHookSettings(ctx, options, targetKey, workflow) {
  ...
  if (options.dryRun) {
    log(`would upsert AS-Workflow hooks -> ${settingsPath}`);
    if (workflow.statusLine) log(`would upsert AS-Workflow statusLine -> ${settingsPath}`);
    return;
  }
  ...
  if (workflow.hookStyle === 'gemini') {
    addHookGroup(settings, 'SessionStart', 'startup|resume|clear', geminiHook('agentic-supercharge-session-start', `${commandBase} --event session-start`));
    addHookGroup(settings, 'BeforeAgent', '*', geminiHook('agentic-supercharge-prompt-submit', `${commandBase} --event prompt-submit`));
    addHookGroup(settings, 'AfterTool', '*', geminiHook('agentic-supercharge-post-tool-use', `${commandBase} --event post-tool-use`));
  } else {
  ...
  log(`upserted AS-Workflow hooks -> ${settingsPath}`);
}
...
function uninstallWorkflowHookSettings(ctx, options, targetKey, workflow) {
  ...
  if (options.dryRun) {
    log(`would remove AS-Workflow hooks -> ${settingsPath}`);
    return;
  }
  const settings = readJsonFile(settingsPath, {});
  const pruned = pruneInvalidCommandHooks(settings);
  ...
  fs.writeFileSync(settingsPath, `${JSON.stringify(settings, null, 2)}\n`);
  if (pruned) log(`removed ${pruned} invalid command hook(s) -> ${settingsPath}`);
  log(`removed AS-Workflow hooks -> ${settingsPath}`);
}
...
function installWorkflowCommands(ctx, options, targetKey, workflow) {
  ...
  for (const [name, body] of Object.entries(commands)) {
    ...
    if (options.dryRun) {
      log(`would upsert AS-Workflow slash command -> ${dest}`);
      continue;
    }
    writeManagedTextFile(dest, body, `AS-Workflow slash command ${name}`);
  }
}

function uninstallWorkflowCommands(ctx, options, workflow) {
  const dir = resolvePath(workflow.commandsDir, ctx);
  for (const name of ['as-status', 'as-resync', 'as-knowledge', 'as-doctor', 'as-checkpoint', 'as-usage']) {
    const dest = path.join(dir, `${name}.md`);
    if (!fs.existsSync(dest)) continue;
    if (options.dryRun) log(`would remove managed AS-Workflow slash command -> ${dest}`);
    else removeManagedTextFile(dest, `removed AS-Workflow slash command -> ${dest}`);
  }
}

function installWorkflowRule(ctx, options, targetKey, workflow) {
  const dir = resolvePath(workflow.rulesDir, ctx);
  const dest = path.join(dir, 'agentic-supercharge-workflow.mdc');
  const body = managedText(`AS-Workflow rule`, `---
description: AgenticSupercharge workflow awareness
alwaysApply: true
---

# AgenticSupercharge Workflow Awareness

For non-trivial project work, prefer the local AS-Workflow state in \`.agenticsupercharge/\` when it exists.

- Read \`.agenticsupercharge/state.md\` or active work only when it helps the current task.
- Read \`.agenticsupercharge/preferences.md\` at the start of meaningful work when it exists. Append short dated do-not/preference notes when the user clearly corrects a repeated mistake, says "never do X", or shows frustration; never store secrets.
- For reference docs, inspect \`.agenticsupercharge/knowledge-index.json\` first, then load only the relevant document or markdown cache.
...
- Do not commit \`.agenticsupercharge/\`; it is local runtime state.
`);
  if (options.dryRun) {
    log(`would upsert AS-Workflow rule -> ${dest}`);
    return;
  }
  writeManagedTextFile(dest, body, `AS-Workflow rule for ${targetKey}`);
}

function uninstallWorkflowRule(ctx, options, workflow) {
  const dest = path.join(resolvePath(workflow.rulesDir, ctx), 'agentic-supercharge-workflow.mdc');
  if (!fs.existsSync(dest)) return;
  if (options.dryRun) log(`would remove managed AS-Workflow rule -> ${dest}`);
  else removeManagedTextFile(dest, `removed AS-Workflow rule -> ${dest}`);
}
...
function workflowShimPath(ctx) {
  return path.join(workflowBaseDir(ctx), 'bin', 'agentic-supercharge');
}

function workflowHookCommand(ctx, targetKey) {
  const cli = path.join(workflowRuntimeCurrentDir(ctx), 'bin/agentic-supercharge.js');
...
function geminiHook(name, command) {
  return {
    ...
    command,
    timeout: 5000,
    description: 'AgenticSupercharge AS-Workflow state helper'
  };
}
...
function isWorkflowHook(hook) {
  return Boolean(hook?.command && isWorkflowCommand(hook.command)) || /^agentic-supercharge-/.test(hook?.name || '');
}

function isWorkflowCommand(command) {
  return String(command || '').includes('agentic-supercharge') && String(command || '').includes('hook');
}

function workflowCommandBodies(ctx, targetKey) {
  const shim = workflowShimPath(ctx);
  return {
    'as-status': managedText('AS-Workflow slash command as-status', `Run \`${shim} status --project-dir "$PWD"\` and summarize the current AS-Workflow state briefly.`),
    'as-resync': managedText('AS-Workflow slash command as-resync', `Run \`${shim} resync --project-dir "$PWD" --dry-run\`. If the user confirms, rerun with \`--apply\`.`),
    'as-knowledge': managedText('AS-Workflow slash command as-knowledge', `Run \`${shim} knowledge --project-dir "$PWD" --dry-run\`. If the user confirms, rerun with \`--apply\` to refresh the local knowledge vault index.`),
    'as-doctor': managedText('AS-Workflow slash command as-doctor', `Run \`${shim} doctor --project-dir "$PWD"\` and explain any workflow issues without changing project code.`),
    'as-checkpoint': managedText('AS-Workflow slash command as-checkpoint', `Run \`${shim} checkpoint --project-dir "$PWD"\` to create a local handoff snapshot, then report the created file.`),
    'as-usage': managedText('AS-Workflow slash command as-usage', `Run \`${shim} usage --project-dir "$PWD"\` to summarize local token usage. Do not print prompt or message content.`)
  };
}

function managedText(label, body) {
  return `<!-- agentic-supercharge:managed:${label} -->\n${body.trimEnd()}\n`;
}

function writeManagedTextFile(dest, body, label) {
  if (fs.existsSync(dest)) {
    const current = fs.readFileSync(dest, 'utf8');
    if (!current.includes('agentic-supercharge:managed')) {
...
function removeManagedTextFile(dest, message) {
  const current = fs.readFileSync(dest, 'utf8');
  if (!current.includes('agentic-supercharge:managed')) return;
...
function writeRuntimeMarker(ctx, runtimeDir, version) {
  const marker = {
    ...
    installedAt: new Date().toISOString(),
    purpose: 'AS-Workflow persistent runtime'
  };
...
function runOptionalToolSetup(ctx, options, setup, skipReason, ops) {
  const base = {
    ...
    status: null,
    reason: null,
    manualCommand: setup.manualCommand(ops.platform),
    skills: setup.skills,
  };
  try {
...
function optionalToolInstallSkipReason(options, env = process.env) {
  ...
  if (String(env.CI || '').toLowerCase() === 'true' && env.AGENTIC_SUPERCHARGE_TOOL_INSTALL !== '1') {
    return 'CI=true detected; rerun outside CI or set AGENTIC_SUPERCHARGE_TOOL_INSTALL=1 to opt in';
  }
  return null;
}
...
function pruneRetiredManagedSkills(ctx, options, plan) {
  ...
  for (const target of plan.skillTargets) {
    ...
    for (const entry of fs.readdirSync(target.skillRootAbs)) {
      ...
      const skillDir = path.join(target.skillRootAbs, entry);
      const markerPath = path.join(skillDir, markerFile);
      if (!fs.existsSync(markerPath)) continue;
      count += 1;
      if (options.dryRun) {
...
function assertManagedMarkerPair(text, targetPath) {
  ...
  if (starts !== ends) {
    throw new Error(`Instruction file ${targetPath} has mismatched AgenticSupercharge managed block markers (${starts} start, ${ends} end). Restore or remove the orphan marker before running this command.`);
  }
  if (starts > 1) {
    throw new Error(`Instruction file ${targetPath} has ${starts} AgenticSupercharge managed blocks. Keep one block or remove duplicates before running this command.`);
  }
}
...
function printWelcome(options, plan) {
  ...
  console.log('');
  console.log(style('AgenticSupercharge is installed and working behind the scenes to improve the quality of what your agent produces.', 'green'));
  console.log('Restart or reload your coding agent so it re-indexes the updated skills.');
  console.log('');
  console.log('Useful commands:');
  console.log(`  ${style('agentic-supercharge status', 'cyan')}      show AS-Workflow project state`);
  console.log(`  ${style('agentic-supercharge doctor', 'cyan')}      check workflow health`);
  console.log(`  ${style('agentic-supercharge resync', 'cyan')}      refresh file-index state (use --apply to write)`);
  console.log(`  ${style('agentic-supercharge knowledge', 'cyan')}   refresh local knowledge vault index`);
  console.log(`  ${style('agentic-supercharge checkpoint', 'cyan')}  create a handoff snapshot`);
  console.log(`  ${style('agentic-supercharge route', 'cyan')}       record a short routing trace when useful`);
  console.log(`  ${style('agentic-supercharge usage', 'cyan')}      show local token usage without prices or telemetry`);
  console.log('');
  ...
  if (plan.scope === 'global') console.log('Hooks are advisory and fail open; your agent should keep working if AS-Workflow is unavailable.');
}
...
Usage:
  ...
  agentic-supercharge update-skills [options]
  agentic-supercharge self-update [options]
  agentic-supercharge check-updates [options]
  agentic-supercharge uninstall [options]
  agentic-supercharge verify [options]
  agentic-supercharge list-targets
  agentic-supercharge status [--project-dir PATH] [--json]
  agentic-supercharge resync [--project-dir PATH] [--apply] [--json]
  agentic-supercharge knowledge [--project-dir PATH] [--apply] [--json]
  agentic-supercharge doctor [--project-dir PATH] [--json]
  agentic-supercharge checkpoint [--project-dir PATH] [--message TEXT]
  agentic-supercharge usage [--project-dir PATH] [--days N|--all] [--json]
...
Install options:
  ...
  --all-skills                      For update-skills: refresh matching skill folders with backup-and-replace.
	  ...
	  --no-tool-install                 Skip optional external tool setup for Graphify, video, and browser helpers.
...
Examples:
  ...
  agentic-supercharge check-updates
  ...
  agentic-supercharge usage
  agentic-supercharge usage --days 7 --json
  agentic-supercharge update-skills --all-skills
