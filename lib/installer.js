    ...
    for (const skill of expectedSkills) {
      ...
      if (fs.existsSync(skillMd)) {
        ...
        if (skill === 'graphify') {
          ...
          check(`${skill} in ${root} has AgenticSupercharge safety note`, skillText.includes('Never install Python packages automatically'));
          check(`${skill} in ${root} does not auto-install Graphify`, !hasUnsafeGraphifyInstallInstruction(skillText));
        }
      }
    }
  }
...
Graphify is optional in AgenticSupercharge. Never install Python packages automatically.

\`\`\`bash
python3 -c "import graphify" 2>/dev/null || graphify --help >/dev/null 2>&1
\`\`\`
...
\`\`\`text
Graphify is not installed. For one-time setup, run:
...
Then continue with normal repository exploration (\`rg\`, file reads, existing AS-Workflow state) unless the user installs Graphify and asks you to rerun the Graphify workflow.
...
function hasUnsafeGraphifyInstallInstruction(text) {
  ...
  return forbiddenBreakFlag.test(text) || /\|\|\s*pip install/i.test(text);
}
...
Design Extract / designlang is optional in AgenticSupercharge. Never install Node packages, Playwright browsers, Chrome, extensions, MCP servers, or global CLIs automatically.

Before running extraction, check whether the tool is already available:

\`\`\`bash
designlang --help >/dev/null 2>&1 || npx --no-install designlang --help >/dev/null 2>&1
\`\`\`

If it is unavailable, tell the user the one-time setup options and wait for them to choose:

\`\`\`text
Design Extract is not available yet. To use it, run one of:
...
Claude Video is optional in AgenticSupercharge. Never install binaries, run Homebrew/apt/winget/pip commands, write API keys, or modify shell/profile files automatically.
...
On non-zero exit, do not run the installer automatically. Tell the user what is missing and offer explicit setup choices:

\`\`\`text
Claude Video needs ffmpeg/ffprobe and yt-dlp for video frames and captions.
Optional Whisper transcription needs a Groq or OpenAI API key configured by you.

You can install dependencies yourself, then rerun /watch.
...
`;
  ...
  text = next
    ...
    .replace(/auto-installs/gi, 'does not install automatically')
    ...
    .replace(/- \*\*Setup preflight failed\*\*[\s\S]*?(?=\n- \*\*)/i, '- **Setup preflight failed** -> explain the missing binaries or key setup. Do not install packages or collect/write API keys automatically; ask the user to configure dependencies locally, or continue with `--no-whisper` when appropriate.');
...
function patchClaudeVideoSetupScript(src, skillName, sourceId) {
  ...
  text = text
    ...
    .replace(/- Never sudo\. On macOS, auto-install via brew\. Elsewhere, print exact commands\./, '- Never sudo and never auto-install. Print exact commands for the user to run.');
