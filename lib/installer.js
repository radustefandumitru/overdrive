...
function parseArgs(argv) {
  ...
  if (args[0] && !args[0].startsWith('-')) {
    ...
    if (['install', 'verify', 'list-targets', 'self-update', 'update-skills', 'uninstall', 'help'].includes(first)) {
      command = first;
    } else {
...
function installOfficialInstallers(ctx, options, plan) {
  ...
  for (const installer of ctx.manifest.officialInstallers || []) {
    try {
      ...
      if (!installer.type || !['gsd-npm-installer', 'playwright-cli-installer'].includes(installer.type)) throw error;
...
function installGsd(ctx, options, plan, installer) {
  ...
  if (options.dryRun) {
    for (const item of runnable) {
      const configDir = resolvePath(item.configDir, ctx);
      log(`would run: npx -y ${packageSpec} ${item.args.join(' ')} --config-dir ${configDir}`);
    }
  ...
  for (const item of runnable) {
    ...
    run('npx', ['-y', packageSpec, ...item.args, '--config-dir', configDir]);
    markGsdRuntime(ctx, configDir, item.runtime, installer, packageSpec);
  }
  mirrorGsdToAgents(ctx, options, plan, installer, packageSpec);
}
...
function markGsdRuntime(ctx, configDir, runtime, installer, packageSpec) {
  ...
  if (fs.existsSync(gsdDir)) writeMarker(ctx, gsdDir, 'get-shit-done-runtime', installer.id, packageSpec, rootMarkerTarget, sourceMeta);
  const skillsDir = path.join(configDir, 'skills');
  if (!fs.existsSync(skillsDir)) return;
  for (const entry of fs.readdirSync(skillsDir).filter((name) => name.startsWith('gsd-'))) {
    const skillDir = path.join(skillsDir, entry);
    if (fs.existsSync(path.join(skillDir, 'SKILL.md'))) {
      writeMarker(ctx, skillDir, entry, installer.id, packageSpec, rootMarkerTarget, sourceMeta);
    }
  }
}

function mirrorGsdToAgents(ctx, options, plan, installer, packageSpec) {
  ...
  for (const name of fs.readdirSync(sourceTarget.skillRootAbs).filter((entry) => entry.startsWith('gsd-')).sort()) {
    ...
    if (fs.existsSync(path.join(src, 'SKILL.md'))) {
      copySkillToTarget(ctx, options, src, name, installer.id, `${sourceTarget.skillRootAbs}/${name}`, agentsTarget, {
        package: installer.package,
        trackingPackage: installer.trackingPackage || null,
        requestedPackage: packageSpec,
        repo: installer.repo,
        mirroredFrom: sourceTarget.key
      });
    }
  }
}
