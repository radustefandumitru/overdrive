...
const { createProgress } = require('./ovd-plan/progress');
...
function parseArgs(argv) {
  ...
  // --install-local is the explicit opt-in for a project-local install (Task 7.6).
  // It implies --scope local so users don't need both flags.
  if (options.installLocal && !options.scope) options.scope = 'local';
...
async function runInstall(ctx, options) {
  ...
  // Presentation pass: quiet-by-default install with a single progress bar over
  // the skill copy. --verbose or --dry-run keeps full per-skill logs (no bar).
  const quietInstall = !options.verbose && !options.dryRun;
  setLogQuiet(quietInstall);
  const totalSkills = countSelectedSkills(ctx, options);
  const installBar = quietInstall ? createProgress(totalSkills, {}) : null;

  installGitSources(ctx, options, plan, installBar);
  installLocalSkills(ctx, options, plan, installBar);
  if (installBar) installBar.done(`✓ Installed ${totalSkills} skill${totalSkills === 1 ? '' : 's'} (${plan.scope})`);
  setLogQuiet(false);

  // Phase 1.5 S3 — for a project (local) install, drop a vendored-skill gitignore
  // carve-out into each project skill root so git stays clean (installed skills are
  // regenerable via `overdrive install`) while authored skills stay tracked + shared.
  if (plan.scope === 'local' && !options.dryRun) {
    const vendored = Array.from(options.selectedSkillNames || []);
    for (const target of plan.skillTargets) {
      if (target.skillRootAbs) writeVendoredSkillsGitignore(target.skillRootAbs, vendored);
    }
  }
...
async function runVerify(ctx, options) {
  ...
  for (const [root, expectedSkills] of expectedByRoot.entries()) {
    ...
    for (const skill of expectedSkills) {
      ...
      if (exactSkillFile) {
        ...
        if (skill === 'web-artifacts-builder') {
          const initScriptPath = path.join(root, skill, 'scripts', 'init-artifact.sh');
          const initScript = fs.existsSync(initScriptPath) ? fs.readFileSync(initScriptPath, 'utf8') : '';
          check(`${skill} in ${root} has Overdrive setup safety note`, skillText.includes('## Overdrive Setup Safety') && skillText.includes('Do not run global package installs'));
          check(`${skill} in ${root} avoids global pnpm install guidance`, !hasUnsafeWebArtifactsInstallInstruction(skillText) && !hasUnsafeWebArtifactsInstallInstruction(initScript));
        }
      }
    }
  }
  ...
  console.log('\nMCP guidance checks');
  const mcpGuide = path.join(ctx.kitDir, 'docs', 'MCP_AND_CONNECTORS.md');
  check('docs/MCP_AND_CONNECTORS.md exists', fs.existsSync(mcpGuide));
  ...
  const releaseDocs = ['SECURITY.md', 'docs/VERIFIED_SOURCES.md', 'CHANGELOG.md', 'docs/SKILLS_TLDR.md', 'docs/AGENTS_OPENAI_YAML.md'];
...
async function resolveInteractiveOptions(ctx, options) {
  ...
  // Task 7.6 — gate non-interactive local installs behind the explicit opt-in.
  // Interactive local is handled by the confirmation prompt below (which sets
  // installLocal), so it is not gated here.
  if (!interactive) assertLocalOptIn(options);
  ...
  try {
    ...
    if (options.scope === 'local') {
      ...
      options.installLocal = true; // explicit interactive opt-in (Task 7.6)
    }
...
async function askScope(rl) {
  ...
  log('   Installs into this repo. Vendored skills are gitignored (regenerable via');
  log('   `overdrive install`); your own authored skills stay tracked + shared with the team.\n');
...
// Task 7.6 — a project already carrying .agents/skills or .cursor/skills is a
// clearly-intentional prior local install; that counts as opt-in.
function hasPreexistingLocalSkills(projectDir) {
  return localSkillTargets.some((target) => {
    try { return fs.existsSync(path.resolve(projectDir, target.skillRoot)); }
    catch (err) { return false; }
  });
}

// Task 7.6 — default install is global-only; a project-local install (which writes
// .agents/ and .cursor/ into the working directory) requires explicit opt-in:
// the --install-local flag, an interactive local choice (sets installLocal), or a
// pre-existing local skill dir. Bare `--scope local` without any of these is
// blocked so the CLI never pollutes a project directory by accident.
function assertLocalOptIn(options) {
  if (options.scope !== 'local') return;
  if (options.installLocal) return;
  const projectDir = path.resolve(options.projectDir || process.cwd());
  if (hasPreexistingLocalSkills(projectDir)) {
    options.installLocal = true;
    return;
  }
  throw new Error(
    'Local project install requires explicit opt-in. Re-run with --install-local. '
    + 'The default install is global-only and never writes into the project directory. '
    + '(A project that already contains .agents/skills or .cursor/skills is treated as opted-in.)'
  );
}
...
function printInstallSummary(ctx, options, plan) {
  ...
  console.log(`Scope: ${plan.scope}${plan.projectDir ? ` (${plan.projectDir})` : ''}`);
  if (plan.scope === 'local') {
    console.log(`Installing into the project directory (opted in via --install-local): ${plan.projectDir}`);
  } else {
    console.log('Installing into user-global agent directories only (no project-directory changes).');
  }
...
function countSelectedSkills(ctx, options) {
  let n = 0;
  for (const source of ctx.manifest.sources || []) {
    n += (source.includes || []).filter((inc) => isSkillSelected(options, inc.to)).length;
  }
  n += (ctx.manifest.localSkills || []).filter((item) => isSkillSelected(options, item.to)).length;
  return n;
}

function installGitSources(ctx, options, plan, bar) {
...
function installLocalSkills(ctx, options, plan, bar) {
...
function prepareSkillSource(src, skillName, sourceId, sourceMeta = {}) {
  ...
  for (const transform of sourceMeta.transforms || []) {
    ...
    } else if (transform === 'overdrive-web-artifacts-safe') {
      applyWebArtifactsSafeTransform(src, path.join(src, 'SKILL.md'), skillName, sourceId);
    } else if (transform === 'overdrive-defuddle-safe') {
      applyDefuddleSafeTransform(path.join(src, 'SKILL.md'), skillName, sourceId);
    } else {
...
function applyWebArtifactsSafeTransform(src, skillPath, skillName, sourceId) {
  let text = fs.readFileSync(skillPath, 'utf8');
  if (!text.includes('## Overdrive Setup Safety')) {
    const note = `## Overdrive Setup Safety

This skill is imported from Anthropic's official examples, but Overdrive keeps setup explicit. Before running \`scripts/init-artifact.sh\`, check whether Node 18+ and \`pnpm\` are already available. If \`pnpm\` is missing, ask the user to install or enable it manually, for example with Corepack or their package manager. Do not run global package installs for the user.

`;
    text = text.replace(/## Quick Start\n/, `${note}## Quick Start\n`);
    text = text.replace('Run the initialization script to create a new React project:', 'After confirming the user wants a new artifact project and setup prerequisites are available, run the initialization script:');
    fs.writeFileSync(skillPath, text);
  }

  const initScriptPath = path.join(src, 'scripts', 'init-artifact.sh');
  if (!fs.existsSync(initScriptPath)) throw new Error(`${sourceId}:${skillName} web artifacts transform could not find scripts/init-artifact.sh`);
  let script = fs.readFileSync(initScriptPath, 'utf8');
  script = script.replace(
    /if ! command -v pnpm &> \/dev\/null; then[\s\S]*?npm install -g pnpm\s*fi/,
    `if ! command -v pnpm &> /dev/null; then
  echo "Error: pnpm is required but was not found."
  echo "Overdrive does not install global Node packages automatically."
  echo "Install or enable pnpm manually, for example with Corepack or your package manager, then rerun this script."
  exit 1
fi`
  );
  fs.writeFileSync(initScriptPath, script);

  const finalText = fs.readFileSync(skillPath, 'utf8');
  const finalScript = fs.readFileSync(initScriptPath, 'utf8');
  if (hasUnsafeWebArtifactsInstallInstruction(finalText) || hasUnsafeWebArtifactsInstallInstruction(finalScript)) {
    throw new Error(`${sourceId}:${skillName} web artifacts safety transform left unsafe global install guidance`);
  }
}

function hasUnsafeWebArtifactsInstallInstruction(text) {
  return /npm\s+install\s+-g|pnpm not found\. Installing pnpm/i.test(text || '');
}

function applyDefuddleSafeTransform(skillPath, skillName, sourceId) {
  let text = fs.readFileSync(skillPath, 'utf8');
  text = text.replace(
    /If not installed:\s*`npm install -g defuddle`/i,
    'If `defuddle` is not installed, tell the user it is optional and ask before setup. Do not run global package installs automatically. Ask the user to install the Defuddle CLI manually with their preferred Node package manager, or fall back to normal web fetch/browser reading.'
  );
  if (!text.includes('## Overdrive Setup Safety')) {
    text = text.replace(
      /## Usage\n/,
      '## Overdrive Setup Safety\n\nDefuddle is optional. Check `defuddle --help` before using it. If it is missing, ask the user before installing anything, and continue with normal web fetch/browser reading when setup is declined or unavailable.\n\n## Usage\n'
    );
  }
  fs.writeFileSync(skillPath, text);
  if (/npm\s+install\s+-g/i.test(text) || /If not installed:/i.test(text)) {
    throw new Error(`${sourceId}:${skillName} Defuddle safety transform left unsafe global install guidance`);
  }
}
...
function copyRuntimeDependencies(ctx, runtimeDir, rootPkg) {
  const queue = Object.keys(rootPkg.dependencies || {}).map((name) => ({ name, searchPaths: [ctx.kitDir] }));
  const copied = new Set();
  while (queue.length > 0) {
    const { name, searchPaths } = queue.shift();
    if (copied.has(name)) continue;
    copied.add(name);
    const src = resolveDependencyRoot(ctx, name, searchPaths);
    if (!fs.existsSync(src)) continue;
    ...
    try {
      ...
      for (const t of Object.keys(depPkg.dependencies || {})) {
        if (!copied.has(t)) queue.push({ name: t, searchPaths: [src, ...searchPaths] });
      }
    } catch (_e) {
...
function resolveDependencyRoot(ctx, name, searchPaths = []) {
  const direct = path.join(ctx.kitDir, 'node_modules', name);
  if (fs.existsSync(direct)) return direct;
  try {
    const pkgPath = require.resolve(`${name}/package.json`, { paths: [...searchPaths, ctx.kitDir] });
...
function uninstallWorkflowRuntime(ctx, options) {
  ...
  removeDirIfEmpty(path.join(base, 'runtime'));
  removeDirIfEmpty(path.join(base, 'bin'));
  removeDirIfEmpty(base);
  log('removed managed ovd-workflow runtime files');
}

// Cosmetic cleanup after uninstall: drop now-empty managed directories
// (~/.overdrive/bin, ~/.overdrive/runtime, ~/.overdrive). Never touches
// directories that still contain anything (e.g. user backups).
...
function installWorkflowHookSettings(ctx, options, targetKey, workflow) {
  ...
  const settings = readUserSettingsFile(settingsPath);
...
// Settings files (Claude/Gemini settings.json, Codex hooks.json) are user-owned.
// If one exists but does not parse, REFUSE to continue rather than fall back to
// `{}` — writing the fallback back would silently destroy the user's settings.
// Mirrors the assertManagedMarkerPair precedent for instruction files.
function readUserSettingsFile(file) {
  ...
  try {
    ...
    throw new Error(
      `${file} exists but is not valid JSON (${error.message}). ` +
      'Overdrive will not modify it in this state because rewriting it could lose your settings. ' +
      'Fix the JSON (or move the file aside), then re-run this command.'
    );
  }
}
...
// LOG_QUIET suppresses the chatty per-skill/per-source install logs when the
// presentation-pass progress bar is active (default install). Summaries use
// console.log directly and are never suppressed. Default false → unchanged for
// every non-install command. Set only by runInstall.
let LOG_QUIET = false;
function setLogQuiet(value) { LOG_QUIET = !!value; }
...
Install options:
  ...
  --install-local                   Opt in to a project-local install (implies --scope local).
                                    Required for any local install; default install is global-only.
...
// Phase 1.5 S3 — vendored/authored gitignore carve-out for project (local) installs.
// Writes a self-contained .gitignore INSIDE the project skill root listing the
// installer-vendored skill folders (regenerable via `overdrive install`). Team-
// authored skills are NOT listed, so they stay tracked and travel through git.
// Deterministic + idempotent (sorted, regenerated each install).
function writeVendoredSkillsGitignore(skillRootAbs, vendoredNames) {
  const lines = [
    '# Overdrive-managed — DO NOT edit by hand.',
    '# Vendored skills below are installed by `overdrive install` and are fully',
    '# regenerable from manifest.json + the lockfile, so they are gitignored.',
    '# Your own (authored) skills are NOT listed here: add a skill folder and it',
    '# stays tracked, so teammates get it on `git pull`.',
    ''
  ];
  const names = Array.from(new Set((vendoredNames || []).filter((n) => typeof n === 'string' && n.trim())))
    .sort();
  for (const name of names) lines.push(`/${name}/`);
  fs.mkdirSync(skillRootAbs, { recursive: true });
  const file = path.join(skillRootAbs, '.gitignore');
...
module.exports = {
  ...
  runCli,
  parseArgs,
  writeVendoredSkillsGitignore,
  assertLocalOptIn,
  hasPreexistingLocalSkills,
  targetDefs,
  buildInstallPlan,
  createContext,
  setupOptionalTools,
