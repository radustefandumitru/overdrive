...
function parseArgs(argv) {
  ...
  if (args[0] && !args[0].startsWith('-')) {
    ...
    if (['install', 'verify', 'list-targets', 'self-update', 'update-skills', 'help'].includes(first)) {
      command = first;
    } else {
...
async function runUpdateSkills(ctx, options) {
  ...
  if (options.allSkills) {
    log('Updating all matching AgenticSupercharge skills from upstream sources.');
    ...
    log('Updating AgenticSupercharge-managed skills from upstream sources.');
