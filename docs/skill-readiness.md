...
| `design-extract` | Optional designlang-compatible tooling, Chrome/Chromium or Playwright, and public page access | The skill is local and works as guidance without requiring any upstream Git source. Installer browser setup is optional. Agents should check availability, prefer `--system-chrome`, avoid extensions/MCP/cookies/auth state, and fall back to screenshots/source inspection or user-provided brand details. |
