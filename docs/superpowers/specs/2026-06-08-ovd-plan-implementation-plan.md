...
**FU-2 — RESEARCH findings → leaf reference (SHIPPED 2026-06-23, TDD — `research.js` + `writer.js`; r3 §10.8):**
...
- **Shipped:** `normalizeResearchEntries` validates `attach_to_leaf` (substantive-only, non-empty node id); `applyResearchSubstantive` validates the target is an existing leaf *before* writing (no orphan files on a bad id), then appends the file path to `references.research[]` via `openState → mutate → commitState` (the committed-tree round-trip; `findNodeById` from cache.js, cycle-safe). `writer.js REFERENCES_KEY_ORDER` gains `research`. Plan-mode text documents the flag. Research suite 204 checks; full suite green (`test:ovd-plan` 5027, smoke 35, consistency 1181, router 269). One-liners stay inbox-only.
...
- **Q4.3.1-followup (Phase 4 Task 4.3 finding — spec citation):** §5 Phase 4 Task 4.3 references "r3 §6.8 table" for the verification-method dispatch, but r3 §6.8 is "Task targeting" — **there is no method-dispatch table in r3**. Methods appear only as examples in leaf annotations (`playwright_visual_regression`, `api_response_check`, `agent_self_check_against_success_criteria`, etc.) and the canonical v1 method set is enumerated in readiness brief 13 Task 4.3 (`agent_self_check_against_success_criteria` always-available + `playwright_visual_regression` / `security-review` / `react-doctor` / `api_response_check` / `unit_test_run`). Task 4.3 ships `verify.js` with that set in `KNOWN_METHODS` + pass-through for non-standard methods. **r4 amendment should either add a real §6.8 (or §6.10) method-dispatch table to r3 OR correct the §5 Task 4.3 citation to point at brief-13 / §5.6.** Minor; non-blocking. Scoped here per Failure Mode #8.
- **Q4.4.1-followup (Phase 4 Task 4.4 finding — classifier locus):** §5 Phase 4 Task 4.4 frames `classifyUserResponse(response)` as a CLI function (unit-tested); readiness brief 13's "structurally novel" section frames classification as **agent-side** (the agent has conversational context the CLI lacks) with the CLI only validating the label (FM #4). Task 4.4 ships a **hybrid that reconciles both**: `review.js` provides a deterministic first-pass `classifyUserResponse` (word-lists + contradiction heuristic; clear → approved/defer/iterate, near-miss → ambiguous) that is fully unit-tested, AND `normalizeReviewEntries` accepts an explicit agent `classification` that is **authoritative** (overrides the first-pass) when provided. Status transitions always go through the writer round-trip (FM #4); only explicit approval → `done` (§10.3 / hard rule 8); ambiguous re-prompts, never auto-promotes. **No r4 change strictly required** (the hybrid honors both framings); an r4 clarification could note that the CLI classifier is a deterministic safety-net and the agent label is authoritative. Scoped here per Failure Mode #8 so the framing difference is recorded.
...
### 2026-06-19 — Session 19 continued (Phase 4 Task 4.3 COMPLETE — LEAF VERIFY dispatch)

**Pre-flight (Methodology 2 — 14th instance) surfaced a spec-citation gap (→ §6 follow-up Q4.3.1).** §5 Task 4.3 cites "r3 §6.8 table" for method dispatch, but §6.8 is "Task targeting" — there is no method table in r3; methods are only annotation examples, and the canonical v1 set lives in brief-13 Task 4.3. Logged Q4.3.1; used brief-13's set. Other picks confirmation-class (Pattern-1 dispatch like execute; report-only, status transitions stay with 4.4/4.9; transparent fallback per Q4.2; availability is agent-side).

**Task 4.3 — `verify.js` (~250 lines), Pattern-1 dispatch.** PLAN (`buildVerifyPlan`) reads `verify.{method,fallback,review_required}` (r3 §5.6 defaults: method/fallback → `agent_self_check_against_success_criteria`, review_required → true) + success_criteria, and emits a verify plan instructing the agent to run the method and **fall back transparently** to the always-available self-check if the method's tool/skill is unavailable (Q4.2 — never fail hard; availability is agent/runtime-side, so the CLI declares method+fallback and the agent reports `method_used`/`fallback_used`). `KNOWN_METHODS` = the brief-13 v1 set; non-standard methods are flagged + passed through. COMMIT (`applyVerifyResult`) validates the verdict (Pattern 4: `normalizeVerifyEntries` — `result ∈ {pass,fail}`, findings filtered), persists a verify report to `.overdrive/sessions/<ts>-verify-<id>.md`, and returns pass/fail + action paths: **pass** → approve/advance (reflecting `review_required`) + re-verify; **fail** → fix / replan / skip per **r3 §6.9**. **Report-only** — no status mutation (AWAITING REVIEW / FIX own transitions); iteration-history persistence (r3 §10.6) wires in with Task 4.5. **Pattern 2 reuse:** `execute.findLeaf`/`leafContract`, `noderef.isLeaf`, `deliberation-state.openState`, `research.isoToFilenameSafe`. **Pattern 1:** CLI does no verification work — the agent runs the method between plan and commit. Wired `/ovd-go verify <ref>` (plan) + `--entries-json` (commit) in `index.js` with the Pattern-4 JSON guard.

**Tests:** `scripts/test-ovd-plan-verify.js` — **84 checks across 8 groups**: module surface; verifySpec (full annotation + no-annotation defaults + review_required:false honored + null-annotation defaults); buildVerifyPlan (method/fallback/review_required/success/callback text + non-standard-method flag + default method + not-a-leaf/leaf-not-found/missing-ref/missing-plan); normalizeVerifyEntries (Pattern 4 matrix incl. invalid/missing result + findings filter + fallback_used); applyVerifyResult (pass action paths + fail §6.9 action paths + review_required:false auto-advance note + fallback note + **session report** + no-findings render + error paths); runVerify dispatch + `ovdPlan.runGo` (plan/commit/bad-JSON); **migration-compat seam (Pattern 5)**.

**Cross-task test fix:** repointed the orient suite's "subcommand → stub" assertion from `verify` to a synthetic `__unimplemented__` subcommand — churn-proof now (any unrecognized subcommand → stub), so future Phase-4 subcommands won't re-break it.

**Estimation (3-factor model):** verify.js hit factor (2) FIXED-SHAPE render (plan template + pass/fail commit templates) + light factor (1) → ~250 lines, in-band. 15th data point. Test count topped from 79 → 84 to clear the Pattern-8 ≥80 bar (genuine edge cases).

**Regression:** `npm run check` exit 0; **ovd-plan 3651 → 3735 checks across 28 suites** (+84 verify, +1 suite); `test:workflow` pass; `eval:router` 269 pass. No regressions.

**Files:** `lib/ovd-plan/verify.js` (new), `scripts/test-ovd-plan-verify.js` (new), `lib/ovd-plan/index.js` (mod — require + verify dispatch + export), `scripts/test-ovd-plan-orient.js` (mod — stub assertion made churn-proof), `package.json` (mod), this §7 entry + §6 Q4.3.1 (doc). Commit boundary proposed at end of this entry.

**Next:** Task 4.4 (AWAITING REVIEW + approval-signal classifier) — the structurally-novel fuzzy-NLP boundary; ~80–120 phrasing-variant tests incl. near-misses (FM #5 over-flagging). Consumes verify pass → AWAITING REVIEW; never auto-promotes (hard rule 8).

### 2026-06-20 — Session 19 continued (Phase 4 Task 4.4 COMPLETE — AWAITING REVIEW + approval classifier)

**Pre-flight (Methodology 2 — 15th instance) surfaced the classifier-locus conflict (→ §6 follow-up Q4.4.1), resolved by a hybrid.** §5 Task 4.4 frames `classifyUserResponse` as a CLI function (unit-tested); brief-13 frames classification as agent-side (it has conversational context), CLI validates the label (FM #4). Hybrid reconciles: CLI ships a deterministic first-pass classifier (unit-testable per impl-plan) AND accepts an authoritative agent `classification` (per brief-13). Logged Q4.4.1.

**Task 4.4 — `review.js` (~250 lines), the first fuzzy-NLP boundary in v1.** `classifyUserResponse(response)` → `{classification, evidence}` over four classes: clean approval (approval token, no contradiction/change) → `approved`; approval + contradiction/change ("looks good **but**…", "approve **once** Y", "done, **except**…") → `iterate` (the requested change wins — FM #5 over-flagging defense); pure change request → `iterate`; defer tokens → `defer`; no clear signal ("ok", "yeah good", "hmm") → `ambiguous`. `matchAny` uses word-boundary regex for single words (so "next" matches but "nextdoor"/"context" don't) and substring for phrases/emoji. `presentForReview` renders the r3 §6.4 prompt (changes + verify result + success + the canonical "'approved' or describe changes" + defer/replan). **Hybrid authority:** `normalizeReviewEntries` takes an explicit agent `classification` (authoritative, `source:'agent'`) OR a raw `response` (→ first-pass, `source:'first-pass'`); the agent label overrides. **Transitions (hard rule 8 / §10.3, via writer round-trip):** `approved` → `done` (+ recursive-close hint, Task 4.6); `iterate` → `in-progress` (+ re-execute hint, Task 4.5); `defer` → no promotion (stays awaiting-review); **`ambiguous` → NO transition + numbered clarifying prompt (never auto-promote).** PLAN (`buildReviewPlan`) sets the leaf → `awaiting-review` (per §10.3, after verify pass) and emits the prompt + callback. Wired `/ovd-go review <ref>` (plan) + `--entries-json` (commit). **Pattern 2 reuse:** `execute.findLeaf`/`leafContract`, `noderef.isLeaf`, `deliberation-state.openState/commitState`. **Pattern 1:** CLI classifier is a deterministic safety-net; the agent (with context) is authoritative.

**Tests:** `scripts/test-ovd-plan-review.js` — **103 checks across 9 groups**: module surface; matchAny (word-boundary vs substring vs emoji + no-false-substring); **classifyUserResponse phrasing tables** — 12 approved + 13 iterate (incl. all FM #5 near-misses) + 7 defer + 8 ambiguous, plus evidence-rule + non-string checks (~42 classifier checks, satisfying Q4.3's "~50 phrasings"); presentForReview (r3 §6.4 prompt shape); buildReviewPlan (awaiting-review persisted + callback + errors); normalizeReviewEntries (agent-authoritative vs first-pass vs missing-signal + agent-overrides-first-pass); applyReviewResponse (approved→done + recursive hint / iterate→in-progress / defer→no-promotion / **ambiguous→no-transition + numbered prompt** / agent-authoritative-iterate-not-done / errors); runReview dispatch + `ovdPlan.runGo` (plan/commit/bad-JSON); **migration-compat seam (Pattern 5)**.

**Estimation (3-factor model):** review.js hit factor (1) classification logic + factor (2) FIXED-SHAPE render (prompt + per-class commit templates) + factor (3) status mutation → ~250 lines, in-band. The heaviness landed in the TEST surface (103 checks — the phrasing tables), not source, as anticipated for the fuzzy-NLP boundary. 16th data point.

**Regression:** `npm run check` exit 0; **ovd-plan 3735 → 3838 checks across 29 suites** (+103 review, +1 suite); `test:workflow` pass; `eval:router` 269 pass. No regressions.

**Files:** `lib/ovd-plan/review.js` (new), `scripts/test-ovd-plan-review.js` (new), `lib/ovd-plan/index.js` (mod — require + review dispatch + 3 exports), `package.json` (mod), this §7 entry + §6 Q4.4.1 (doc). Commit boundary proposed at end of this entry.
...
### 2026-06-21 — Session 20 continued (Phase 5 Task 5.2 Slice B COMPLETE — HANDOFF steps 6–10)
